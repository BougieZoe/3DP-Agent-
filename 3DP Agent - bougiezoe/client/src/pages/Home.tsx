...
export default function Home() {
  ...
  const getModelData = (): ModelData | null => {
    ...
    const geo = uploadedModel.analysis.geometry;
    if (!geo) return null;
    return {
      fileName: uploadedModel.fileName,
      wallThickness: { status: 'good', minThickness: 0, averageThickness: 0, sampledPoints: 0, areas: 0, method: 'estimated' },
      overhang: { status: 'good', angle: 45, areas: 0, maxAngle: 0, area: 0 },
      volume: geo.volume * 1000,
      boundingBoxVolume: geo.volume * 1000,
      surfaceArea: geo.surfaceArea * 100,
      mesh: { faceCount: geo.triangleCount, isWatertight: geo.isManifold, vertexCount: geo.vertexCount, degenerateFaces: 0, boundaryEdges: 0, nonManifoldEdges: 0, centerOfMass: { x: 0, y: 0, z: 0 } },
      holes: { count: 0, boundaryLoops: [] },
      dims: {
        x: geo.dimensions.width,
        y: geo.dimensions.depth,
        z: geo.dimensions.height,
      },
    };
  };
  ...
  const providerLabel = getActiveProvider() ? { claude: 'Claude', openai: 'GPT-5.5', gemini: 'Gemini', deepseek: 'DeepSeek' }[getActiveProvider()!] : null;
  return (
                  ...
                  <div className="space-y-4 pt-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 border border-border rounded-sm bg-card">
                        <div className="text-xs text-muted-foreground mb-2 font-mono">PRINTABILITY</div>
                        <StatusChip status={print?.score === 'poor' ? 'critical' : print?.score === 'fair' ? 'warning' : 'good'} />
                      </div>
                      <div className="p-3 border border-border rounded-sm bg-card">
                        <div className="text-xs text-muted-foreground mb-2 font-mono">ISSUES</div>
                        <StatusChip status={(print?.errorCount ?? 0) > 0 ? 'critical' : (print?.warningCount ?? 0) > 0 ? 'warning' : 'good'} />
                      </div>
                    </div>
                    <div className="border border-border rounded-sm bg-card p-4">
                      <div className="text-xs text-muted-foreground mb-3 font-mono tracking-widest">GEOMETRY DATA</div>
                      <MetricRow label="TRIANGLES" value={geo?.triangleCount ?? '-'} />
                      <MetricRow label="VOLUME" value={geo ? (geo.volume).toFixed(2) : '-'} unit="cm³" highlight />
                      <MetricRow label="SURFACE AREA" value={geo ? (geo.surfaceArea).toFixed(2) : '-'} unit="cm²" />
                      <MetricRow label="DIM X" value={geo ? geo.dimensions.width.toFixed(2) : '-'} unit="mm" />
                      <MetricRow label="DIM Y" value={geo ? geo.dimensions.depth.toFixed(2) : '-'} unit="mm" />
                      <MetricRow label="DIM Z" value={geo ? geo.dimensions.height.toFixed(2) : '-'} unit="mm" />
                      <MetricRow label="ERRORS" value={print?.errorCount ?? 0} />
                      <MetricRow label="WARNINGS" value={print?.warningCount ?? 0} />
                      <MetricRow label="PRINTABLE" value={print?.printable ? 'YES' : 'NO'} />
                    </div>
