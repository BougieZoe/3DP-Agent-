...
import { ModelData, classifyQuestion, answerLocally, localGeometryAnswer } from '@/lib/ruleEngine';
...
function buildSystemPrompt(model: ModelData, lang: Language): string {
  return `You are an expert 3D printing engineer. Answer based on the parsed STL geometry data below, not generic 3D printing advice.

Rules:
- Cite the actual model measurements when making a recommendation.
- If the data is approximate or sampled, say so briefly.
- Do not invent unseen features. For holes, only treat "boundary loops" as detected open STL openings; watertight through-holes may require slicer/CAD verification.
- Keep the answer concise, technical, and actionable.
- Reply in ${lang === 'zh' ? 'Chinese (Simplified)' : lang === 'ja' ? 'Japanese' : 'English'}.

Parsed STL geometry context:
${buildGeometryContext(model)}`;
}

function buildGeometryContext(model: ModelData): string {
  const holes = model.holes?.boundaryLoops ?? [];
  const overhangPoints = model.overhang.samplePoints ?? [];

  return [
    `File: ${model.fileName}`,
    `Dimensions: ${model.dims.x.toFixed(2)} x ${model.dims.y.toFixed(2)} x ${model.dims.z.toFixed(2)} mm`,
    `Actual mesh volume: ${model.volume.toFixed(2)} mm^3`,
    `Bounding-box volume: ${model.boundingBoxVolume?.toFixed(2) ?? 'n/a'} mm^3`,
    `Surface area: ${model.surfaceArea.toFixed(2)} mm^2`,
    `Wall thickness: min ${model.wallThickness.minThickness.toFixed(3)} mm, avg ${fmt(model.wallThickness.averageThickness)} mm, status ${model.wallThickness.status}, samples ${model.wallThickness.sampledPoints ?? 0}, method ${model.wallThickness.method ?? 'unknown'}`,
    `Overhangs: threshold ${model.overhang.angle} deg, faces ${model.overhang.areas}, max ${fmt(model.overhang.maxAngle)} deg, avg ${fmt(model.overhang.averageAngle)} deg, face ratio ${fmt(model.overhang.faceRatio)}, area ${fmt(model.overhang.area)} mm^2, status ${model.overhang.status}`,
    `Overhang sample points: ${overhangPoints.length ? overhangPoints.slice(0, 5).map(point => `(${point.x.toFixed(2)}, ${point.y.toFixed(2)}, ${point.z.toFixed(2)}) ${point.angle.toFixed(1)}deg`).join('; ') : 'none detected'}`,
    `Mesh: ${model.mesh?.faceCount ?? 0} triangles, ${model.mesh?.vertexCount ?? 0} vertices, degenerate faces ${model.mesh?.degenerateFaces ?? 0}, watertight ${model.mesh?.isWatertight ? 'yes' : 'no'}, boundary edges ${model.mesh?.boundaryEdges ?? 0}, non-manifold edges ${model.mesh?.nonManifoldEdges ?? 0}`,
    `Center of mass: ${model.mesh ? `(${model.mesh.centerOfMass.x.toFixed(2)}, ${model.mesh.centerOfMass.y.toFixed(2)}, ${model.mesh.centerOfMass.z.toFixed(2)}) mm` : 'n/a'}`,
    `Detected open boundary holes/openings: ${holes.length}`,
    holes.length
      ? `Hole/opening details: ${holes.slice(0, 8).map((hole, index) => `#${index + 1} center (${hole.center.x.toFixed(2)}, ${hole.center.y.toFixed(2)}, ${hole.center.z.toFixed(2)}) mm, diameter ${hole.diameter.toFixed(2)} mm, axis (${hole.axis.x.toFixed(2)}, ${hole.axis.y.toFixed(2)}, ${hole.axis.z.toFixed(2)}), vertices ${hole.vertices}`).join('; ')}`
      : 'Hole/opening details: no open boundary loops detected',
  ].join('\n');
}
  ...
  const sendMessage = async (text: string) => {
    ...
    if (!provider) {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString() + '_a',
        role: 'assistant',
        content: localGeometryAnswer(model, language),
        source: 'local',
      }]);
      setIsLoading(false);
      return;
